		...
		public function load($load_object, $ignores = array())
		{
			...
			foreach ($this->_props as $field_name => $val)
			{
				...
				if (!in_array($field_name, $ignores)) {
					...
					if ($exists) {
						$val = $load->$l_field_name;
						if (is_array($val)) // checkbox
							$val = _arr2bits($val);
					}
					else {
						// for single checkbox
						...
						$exists = property_exists($load, $l_field_name) || 
							$_exist_prop && $load->exist_prop($l_field_name);
						if ($exists) {
							$val = _arr2bits($load->$l_field_name);
						}
					}

					if ($exists) {
						if ($this->$field_name != $val) // set dirty
							$this->_dirties[] = $field_name;
						$this->$field_name = $val;
					}
				}
			}
