<section>
	<div class="page-bar">
		<ul class="page-breadcrumb">
			...
			<li>
				<a href="javascript:;"><?php l("审核注册翻译");?></a>
				<i class="fa fa-angle-right"></i>
			</li>
			<li>
	...
	<form id="list_form" class="table-container" method="post">
		...
		<div class="row margin-bottom-10">
			...
			<div class="col-sm-4 text-right">
				<a href="reguser/interpreters/<?php p(RSTATUS_NONE); ?>" class="<?php if($this->status == RSTATUS_NONE) { ?>action-link<?php } ?>"><?php l("待审核"); ?></a>
				|
				<a href="reguser/interpreters/<?php p(RSTATUS_REJECT); ?>" class="<?php if($this->status == RSTATUS_REJECT) { ?>action-link<?php } ?>"><?php l("审核未通过"); ?></a>
			</div>
		</div>
			...
			?>
				<tr reguser_id="<?php p($reguser->reguser_id); ?>">
					...
					<td class="text-center">
						<a href="reguser/detail/<?php p($reguser->reguser_id); ?>/ui">
							<img id="img_cavartar" src="<?php p(_avartar_url($reguser->avartar_id())); ?>" class="small-avartar">
							<?php $reguser->detail("user_name"); ?>
						</a>
					</td>
					<td class="td-sex"><?php $reguser->detail_code("sex", CODE_SEX); ?></td>
					<td class="text-center"><?php $reguser->detail("mobile"); ?></td>
					<td class="text-center"><?php $reguser->detail("email"); ?></td>
					<td class="text-center"><?php $reguser->detail("i_age"); l("年"); ?></td>
					<td class="text-center"><?php $reguser->detail("languages"); ?></td>
					<?php if ($this->status == RSTATUS_NONE) { ?>
					<td class="td-action">
						<a href="reguser/detail/<?php p($reguser->reguser_id); ?>/ui" class="btn btn-xs btn-primary"><?php l("审核"); ?></a>
					</td>
					<?php } else { ?>
		...
		<?php $this->pagebar->display(_url("reguser/interpreters/" . $this->status)); ?>
