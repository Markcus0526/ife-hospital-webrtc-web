...
var App = function () {
    ...
    var callAPI = function(url, params) {
        ...
        .fail(function(ret) {
            ret = {
                err_code: -1,
                err_msg: g_messages.ERR_FAIL_CALL_SERVER
            }
    ...
    var handleFancybox = function (parent) {
        if (!jQuery.fancybox) {
            return;
        }
        ...
        jQuery(parent + ".fancybox").each(function() {
            $this = $(this);
        ...
        iw = $(window).width() - 60;
        ih = $(window).height() - 60;
        jQuery(parent + ".fancybox-image").fancybox({
            'type' : 'iframe',
            'width' : iw,
            'height' : ih,
            'autoSize': false,
        ...
        if (jQuery(parent + ".fancybox-button").size() > 0) {            
            jQuery(parent + ".fancybox-button").fancybox({
                groupAttr: 'data-rel',
                prevEffect: 'none',
                nextEffect: 'none',
                closeBtn: true,
            ...
            $(parent + '.fancybox-video').fancybox({
                type: 'iframe'
            });
        }
    }
    ...
    var handleCycle = function() {
        if ($.fn.cycle) {
            ...
            if ($('.notice-list').length) {
                $(window).resize(function() {
                    var lw = $('.page-logo').width();
                    var tw = $('.top-menu').width();

                    var ww = $(window).width();

                    $('.notice-list').width(ww - lw - tw - 150);
                });
                ...
                if ($('.notice-list li').length > 1) {
                    ...
                    $('.cyclebar ul li').live('hover', function() { 
                        $('.cyclebar ul').cycle('toggle'); 
                    }); 
                }
                else {
                    $('.notice-list li').css('display', 'block');
                }
            }
        }
    }
    ...
    ////////////////////////////////////////
    var handleDatePickers = function (lang) {
        if (lang == undefined)
            lang = "zh-CN";
    ...
    var handleSendPasskey = function () {
        $('.send-passkey').click(function() {
            var $this = $(this);
            mobile_field = $this.attr('mobile');
            var mobile_control = $this.parents('form')
                .find('[name="' + mobile_field + '"]');
            mobile_control.prop('btn_passkey', $this);
            intlTelInput = mobile_control.data('plugin_intlTelInput');
            if(intlTelInput) {
                mobile = intlTelInput.getNumber();
                dialCode = "+" + intlTelInput.getSelectedCountryData().dialCode;
                if (mobile == dialCode)
                    mobile = "";
            }
            else 
                mobile = mobile_control.val();

            if (mobile == '') {
                mobile_control.focus();
                $(mobile_control).valid();
                return;
            }
            ...
            $this.prop('timeout', '');
            $this.find('span').html("...");
            var text_passkey = $this.closest('.form-group').find('input[type="text"]');
            $this.prop('text_passkey', text_passkey);
            text_passkey.val('');
            text_passkey.prop('readonly', true);

            callAPI('api/common/send_passkey', {
                phone_num: mobile
            })
            .done(function(res) {
                alertBox(g_messages.ALERT, g_messages.ERR_SUCCESS_SEND_PASSKYE, null, 5000);
                if ($this.prop('disabled')) {
                    $this.prop('timeout', timer);
                    $this.find('span').html("(" + timer + "s)");
                    $this.prop('text_passkey').prop('readonly', false);
                    $this.closest('.form-group').find('.help-block-error').remove();
                }
            })
            .fail(function(res) {
                errorBox(g_messages.FAILURE, g_messages.ERR_FAIL_SEND_PASSKEY);
                if ($this.prop('disabled')) {
                    $this.prop('timeout', timer);
                    $this.find('span').html("(" + timer + "s)");
                    $this.prop('text_passkey').prop('readonly', false);
            ...
            mobile_control.change(function() {
                var btn_passkey = $(this).prop('btn_passkey');
                btn_passkey.prop('timeout', 0);
                btn_passkey.prop('disabled', false);
                btn_passkey.find('span').html("");
                btn_passkey.closest('.form-group').find('input[type="text"]').val('');
            });
        });

        if ($('.send-passkey').length)
        {
            setInterval(function() {
                $('.send-passkey').each(function() {
                    var $this = $(this);
                    if ($this.prop('disabled')) {
                        ...
                        if (timeout < 0) {
                            ...
                            $this.find('span').html('');
                            var passkey = $this.closest('.form-group').find('input[type="text"]');
                            passkey.attr('timeout', true);
                            //passkey.val('');
                            passkey.valid();
                            return;
                        }
    ...
    var handleIntlTelInput = function() {
        var reformatNumber = function(el, with_dial) {
            if (with_dial === undefined)
                with_dial = true;
            intlTelInput = el.data('plugin_intlTelInput');
            fullNumber = intlTelInput.getNumber();
            prefix = "+" + intlTelInput.selectedCountryData.dialCode;
            val = el.val();

            if (fullNumber.indexOf(prefix) == 0) {
                strip_number = fullNumber.substring(prefix.length);
                if (with_dial == false)
                    el.val(strip_number);
            }

            if (with_dial) {
                if (fullNumber == prefix)
                    fullNumber = "";
                el.val(fullNumber);
            }
        }
        $(".input-intl-tel").each(function() {
            $(this).intlTelInput({
                // allowDropdown: false,
                ...
                geoIpLookup: function(callback) {
                    ...
                    var countryCode = (resp && resp.country) ? resp.country.toLowerCase() : "cn";
                    callback(countryCode);
                  });
                },
                //hiddenInput: name_full,
                initialCountry: "auto",
                nationalMode: true,
            ...
            $(this).on('change', function() {
                reformatNumber($(this), false);
                $(this).valid();
            });
            $(this).parents('form').addClass('intl-tel-form');

            reformatNumber($(this), false);
        });

        $('.intl-tel-form').on('submit', function() {
            $(this).find('.input-intl-tel').each(function() {
                reformatNumber($(this));
            });

            setTimeout("$('.intl-tel-form').resetTelNumber()", 10);
        });

        $.fn.extend({
            resetTelNumber: function() {
                $(this).find('.input-intl-tel').each(function() {
                    reformatNumber($(this), false);
                });
            }
    ...
    var handleAjaxForm = function() {
        if ($.fn.ajaxForm) {
            orgAjaxForm = $.fn.ajaxForm;
            // prevent duplicate call
            $.extend($.fn, {
                ajaxForm: function(options) {
                    var orgBeforeSubmit = null;
                    if (options.beforeSubmit)
                        orgBeforeSubmit = options.beforeSubmit;
                    options.beforeSubmit = function() {
                        var ret = true;
                        if (orgBeforeSubmit) {
                            ret = orgBeforeSubmit.apply(this, arguments);
                            if (ret === false)
                                return ret;
                        }
                    ...
                    return orgAjaxForm.apply(this, [options]);    
                }
            });
        }
    }
    ...
    return {
        ...
        initPopup: function (homeBase, lang) {
            handleInit(homeBase);
