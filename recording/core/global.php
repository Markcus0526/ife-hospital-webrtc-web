<?php
	...
	************************** Copyright Info **************************/
	...
	define('PRODUCT_NAME',		'MARKCUS全球远程医疗会诊系统');
	...
	define('VIDEO_PATH',		DATA_PATH . VIDEO_URL);
	...
	define('MAX_GAP_VIDEOS', 	10 * 60); // 10 minutes
	define('I_EXT',	'webm');
	define('O_EXT',	'mp4');
	...
	define("MAIL_HEADER", "MARKCUS全球远程医疗会诊系统\n你好！\n");
	define("MAIL_FOOTER", "--「MARKCUS全球远程医疗会诊系统」--");
	...
	$g_timezone = null; 
	if (_request("browser_timestamp") != null)
		_reset_timezone();
	else
	...
	function _seconds_label($seconds)
	{
		if ($seconds == null)
			$s = "&nbsp;";
		else {
	...
	{
		global $g_timezone;
		$browser_timestamp = _request("browser_timestamp");
		$reg = '/(GMT\+\d{3,4})/i';
		preg_match($reg, $browser_timestamp, $matches);

		if (isset($matches[1])) {
			_time_zone($matches[1]);
		}
	}
	...
	function _time_zone($time_zone = null, $mysql_mode = false)
	{
		...
		if ($time_zone == null) { // read
			...
			if ($mysql_mode) {
				$mysql_timezone = preg_replace("/(GMT)\+(\d{1,2})(\d{2})$/", "+$2:$3", $g_timezone);
	...
	function _video_url($interview_id, $dir_only=false)
	{
		$dir = VIDEO_URL . substr($interview_id, 0, 4) . "/" . substr($interview_id, 4, 2) . "/";
		if ($dir_only)
			return $dir;

		return $dir . $interview_id . "." . O_EXT;
	}
	...
	// send sms message
	function _push_sms($mobiles, $message)
	{
		if ($mobiles == null)
			return ERR_OK;
		...
		foreach ($mobiles as $mobile) {
			$err = smsModel::push($mobile, $message);
			if ($err != ERR_OK)
				return $err;
		}
		return ERR_OK;
	...
	function _send_sms($phonenum, $message)
	{
		$ret = false;
		if (SMS_ENABLE == ENABLED) {
			...
			$ret = smsManager::sendSMS($phonenum, $message);
		}
	...
	function _l($str, $p1=null, $p2=null, $p3=null, $p4=null) {
		global $g_string;
		$lstr = isset($g_string[$str]) ? $g_string[$str] : null;
		$lstr = ($lstr == null ? $str : $lstr);
		return sprintf($lstr, $p1, $p2, $p3, $p4);
	}

	function l($str, $p1=null, $p2=null, $p3=null, $p4=null) {
		print _l($str, $p1, $p2, $p3, $p4);
	}
