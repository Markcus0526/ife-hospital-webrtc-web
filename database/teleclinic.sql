...
) ENGINE=InnoDB AUTO_INCREMENT=5802 DEFAULT CHARSET=utf8;
...
-- ----------------------------
-- Table structure for `l_logfail`
-- ----------------------------
DROP TABLE IF EXISTS `l_logfail`;
...
CREATE TABLE `m_priv` (
  ...
  `priv_syscheck` int(11) DEFAULT NULL,
...
-- ----------------------------
-- Table structure for `m_string`
-- ----------------------------
DROP TABLE IF EXISTS `m_string`;
...
-- ----------------------------
-- Table structure for `m_uhospital`
-- ----------------------------
DROP TABLE IF EXISTS `m_uhospital`;
CREATE TABLE `m_uhospital` (
  ...
  UNIQUE KEY `uhospital` (`user_id`,`hospital_id`) USING BTREE
...
CREATE TABLE `m_user` (
  ...
  `d_depart` varchar(255) DEFAULT NULL,
...
CREATE TABLE `t_cattach` (
  `cattach_id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `chistory_id` bigint(20) NOT NULL,
...
CREATE TABLE `t_interview` (
  ...
  `reject_cause_id` tinyint(4) DEFAULT NULL,
  `reject_cause_note` text,
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_udisease` AS select `u`.`user_id` AS `user_id`,`d`.`disease_id` AS `disease_id`,`d`.`disease_name` AS `disease_name`,`d`.`disease_name_l` AS `disease_name_l` from (`m_udisease` `u` join `m_disease` `d` on((`u`.`disease_id` = `d`.`disease_id`)));

-- ----------------------------
-- View structure for `v_uhospital`
-- ----------------------------
DROP VIEW IF EXISTS `v_uhospital`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_uhospital` AS select `u`.`user_id` AS `user_id`,`h`.`hospital_id` AS `hospital_id`,`h`.`hospital_name` AS `hospital_name`,`h`.`hospital_name_l` AS `hospital_name_l`,`h`.`sort` AS `sort`,`h`.`hcountry_id` AS `hcountry_id` from (`m_uhospital` `u` join `m_hospital` `h` on((`u`.`hospital_id` = `h`.`hospital_id`)));

-- ----------------------------
-- View structure for `v_ulang`
-- ----------------------------
DROP VIEW IF EXISTS `v_ulang`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ulang` AS select `u`.`user_id` AS `user_id`,`l`.`language_id` AS `language_id`,`l`.`language_name` AS `language_name`,`l`.`language_name_l` AS `language_name_l`,`l`.`sort` AS `sort` from (`m_ulang` `u` join `m_language` `l` on((`u`.`language_id` = `l`.`language_id`)));
